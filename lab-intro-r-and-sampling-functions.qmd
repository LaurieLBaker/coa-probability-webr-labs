...
game_rolls <- data.frame(roll_1 = sample(die_1, size = 100, replace = TRUE),
                         roll_2 = sample(die_2, size = 100, replace = TRUE))

ggplot(game_rolls) + 
  geom_histogram(aes(x = roll_1 + roll_2)) +
  scale_x_discrete(name = "sum of dice", breaks = seq(from = 2, to = 12, by = 1))
```
